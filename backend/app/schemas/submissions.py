...
class SubmissionBlockResponse(BaseModel):
    ...
    created_at: datetime
    is_active: bool = Field(..., description="Whether the block is currently active")
...
class SubmissionStatsResponse(BaseModel):
    ...
    fastest_solve_time: Optional[int] = Field(
        None, description="Fastest solve time in minutes"
    )
    recent_activity: List[SubmissionResponse] = Field(
        default_factory=list, description="Recent submissions"
    )
...
class TeamSubmissionStats(BaseModel):
    ...
    challenges_by_category: dict = Field(
        default_factory=dict, description="Solved challenges grouped by category"
    )
...
class FirstBloodResponse(BaseModel):
    ...
    time_to_solve: Optional[int] = Field(
        None, description="Time to solve from challenge release (in minutes)"
    )
