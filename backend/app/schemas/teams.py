...
class TeamDetailResponse(TeamResponse):
    ...
    recent_solves: Optional[int] = Field(
        default=0,
        description="Number of challenges solved recently"
    )
    rank: Optional[int] = Field(
        default=None,
        description="Team's current rank in the competition"
    )
...
class TeamLeaveRequest(BaseModel):
    ...
    confirm: bool = Field(
        ...,
        description="Confirmation to leave the team",
        examples=[True]
    )
...
class TeamTransferCaptain(BaseModel):
    ...
    confirm: bool = Field(
        ...,
        description="Confirmation to transfer captain role",
        examples=[True]
    )
...
class TeamStatsResponse(BaseModel):
    ...
    avg_solve_time: Optional[float] = Field(
        None,
        description="Average time to solve a challenge (in minutes)"
    )
    category_breakdown: dict = Field(
        default_factory=dict,
        description="Solved challenges by category"
    )
