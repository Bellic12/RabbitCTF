...
class EventConfigCreate(EventConfigBase):
    ...
    start_time: Optional[datetime] = Field(
        None, description="Event start time", examples=["2025-12-01T00:00:00Z"]
    )
    end_time: Optional[datetime] = Field(
        None, description="Event end time", examples=["2025-12-31T23:59:59Z"]
    )
    max_team_size: int = Field(
        default=4, ge=1, le=10, description="Maximum team size", examples=[4]
    )
...
class NotificationCreate(NotificationBase):
    ...
    is_published: bool = Field(
        default=True, description="Whether to publish immediately", examples=[True]
    )
...
class DifficultyResponse(BaseModel):
    ...
    challenge_count: Optional[int] = Field(
        default=0, description="Number of challenges with this difficulty"
    )
...
class ErrorResponse(BaseModel):
    ...
    detail: str = Field(
        ..., description="Error detail message", examples=["Invalid credentials"]
    )
    error_code: Optional[str] = Field(
        None, description="Error code for client handling", examples=["AUTH_001"]
    )
